...
class RotaryEmbeddingFallback(nn.Module):
    ...
    def forward(
        self, position_ids_1d: torch.Tensor
...
class DecoderCoreWrapper(nn.Module):
    # input_ids(B,S), audio_features(B,A,H), attention_mask(B,S)
    # cache_position(S), caches... -> logits + kv_deltas
    def __init__(
        self,
        thinker: nn.Module,
        audio_token_id: int,
    ...
    def _attn_chunk(
        ...
        q = onnx_ops.reshape_from_tensor_shape(
            q, torch.stack([B, S, t_num_heads, t_head_dim])
        )
        k_kv = onnx_ops.reshape_from_tensor_shape(
            k, torch.stack([B, S, t_num_kv_heads, t_head_dim])
        )
        v_kv = onnx_ops.reshape_from_tensor_shape(
            v, torch.stack([B, S, t_num_kv_heads, t_head_dim])
        )
        ...
        cache_k_h = cache_k.permute(0, 2, 1, 3).repeat_interleave(
            self.group_size, dim=1
        )
        cache_v_h = cache_v.permute(0, 2, 1, 3).repeat_interleave(
            self.group_size, dim=1
        )
        ...
        scores_cache = (
            torch.matmul(q.float(), cache_k_h.float().transpose(-1, -2)) * scale
        )
        scores_new = (
        ...
        causal_new = (
            torch.where(
                causal_ok,
                torch.zeros(
                    (), device=scores_new.device, dtype=scores_new.dtype
                ),
                torch.full(
                    (), -1e4, device=scores_new.device, dtype=scores_new.dtype
                ),
            )
            .unsqueeze(0)
            .unsqueeze(0)
        ...
        out = onnx_ops.reshape_from_tensor_shape(
            out, torch.stack([B, S, t_qkv_dim])
        )
    ...
    def forward(
        ...
        *cache_flat: torch.Tensor,
        ...
        if len(cache_flat) != 2 * L:
            raise RuntimeError(
                f"expect {2 * L} cache tensors, got {len(cache_flat)}"
            )
